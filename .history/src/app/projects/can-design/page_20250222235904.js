...
export default function f1Poster() {
  ...
  const dielines = {
    ...
    "Dark-Tea": "/images/projects/cans/Dark-Tea.svg",
      ...
      <div className={styles.contentContainer}>
        ...
        <div className={styles.additionalImages}>
          ...
          {Object.keys(dielines).map((key) => (
            <button
              key={key}
              className="relative pb-1 text-lg transition-all duration-300"
              onClick={() => setSelectedDieline(key)}
            >
              {key.replace("-", " ")} {/* Format the name */}
              <span
                className={`absolute left-0 bottom-0 h-[2px] w-0 bg-black transition-all duration-300 ${
                  selectedDieline === key ? "w-full" : "hover:w-full"
                }`}
              ></span>
            </button>
          ))}
