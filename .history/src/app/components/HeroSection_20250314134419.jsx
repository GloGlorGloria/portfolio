...
const HeroSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > window.innerHeight * 0.3) {
        setShowLanding(false); // Only disappears when user scrolls down
      }
    };
  ...
  return (
    <section>
      {/* 🚀 Fullscreen Landing Page (Disappears After Scrolling) */}
      {showLanding && (
        <motion.div
      ...
      {/* 🔥 Hero Section Appears After Scrolling */}
      <div className={`relative min-h-screen ${showLanding ? "opacity-0" : "opacity-100"} transition-opacity duration-800`}>
