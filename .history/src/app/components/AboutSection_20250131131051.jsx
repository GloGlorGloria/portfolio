"use client";
import React, { useTransition, useState } from 'react';
import Image from 'next/image';
import styles from "./AboutSection.module.css";
import TabButton from './TabButton';

const TAB_DATA = [
    {
        title: "Skills",
        id: "skills",
        content: (
            <ul>
                <li>Adobe Creative Suite</li>
                <li>Figma, User research, Usability Testing</li>
                <li>Google Analytics, Branding, Content strategy</li>
                <li>HTML & CSS, JavaScript, WordPress, React, Agile Methodology</li>
            </ul>
        )
    },
    {
        title: "Education",
        id: "education",
        content: (
            <ul>
                <li> Digital Design and Development Diploma, British Columbia Institute of Technology</li>
                <li> Bachelor of Arts (Honours) in Social Policy & Administration, The Hong Kong Polytechnic University </li>
            </ul>
        )
    }
    {
        title: "Work Experience",
        id: "work experience",
        content: (
            <ul>
                <li> Information Officer (Media Liaison), Fire Services Department, Hong Kong Government </li>
                <li> Assistant Information Officer, Information Services Department, Hong Kong Government </li>
                <li> Research Assistant, The Hong Kong Polytechnic University </li>
            </ul>
        )
    }
]
const AboutSection = () => {
    const [tab, setTab] = useState("skills");
    const [isPending, startTransition] = useTransition();

    const handleTabChange = (id) => {
        startTransition(() => {
            setTab(id);
        });
    };

    return (
        <section>
            {/* <div className={styles.about__container}> */}
            <div className="md:grid md:grid-cols-2 gap-8 items-center py-8 px-4 xl: gap-16 sm:py-16 xl:px-16">
                <Image src={"/images/aboutme.webp"} alt="An AI generated picture for Gloria." width={500} height={500}/>
                <div>
                    <h2 className="font-bold mb-4">About Me</h2>
                    <p className="text-justify">I’m a digital designer with a background in government communications and research, crafting human-centered experiences in UX/UI, graphic design, and digital marketing. I thrive on solving problems, understanding people’s stories, and collaborating with teams to create meaningful solutions. With a cross-cultural perspective, I bring adaptability and innovation to every project. </p>
                <div className="flex flex-row mt-8">
                    <TabButton 
                        selectTab={() => handleTabChange("skills")} 
                        active={tab === "skills"}
                    >
                        {" "}
                        Skills{" "}
                    </TabButton>

                    <TabButton 
                        selectTab={() => handleTabChange("Education")} 
                        active={tab === "Education"}
                    >
                        {" "}
                        Education{" "}
                    </TabButton>

                    <TabButton 
                        selectTab={() => handleTabChange("Work Experience")} 
                        active={tab === "Work Experience"}
                    >
                        {" "}
                        Work Experience{" "}
                    </TabButton>
                    </div>
                </div>
            </div>
        </section>

    );
};

export default AboutSection;