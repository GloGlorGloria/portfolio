...
const HeroSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > window.innerHeight * 0.8) {
        setIsScrolled(true);
        document.body.style.overflowY = "auto"; // Allow scrolling after animation
      } else {
        setIsScrolled(false);
        document.body.style.overflowY = "hidden"; // Lock scrolling during intro
      }
    };

    window.addEventListener("scroll", handleScroll);
    document.body.style.overflowY = "hidden"; // Initial state: no scrolling

    return () => {
      window.removeEventListener("scroll", handleScroll);
      document.body.style.overflowY = "auto"; // Restore scrolling when unmounting
    };
  }, []);
  ...
  return (
    <section>
      ...
      <motion.div
        className="h-screen flex flex-col items-center justify-center text-center bg-[#fdf6ec] w-full fixed top-0 left-0 right-0 z-10"
        initial={{ opacity: 1 }}
